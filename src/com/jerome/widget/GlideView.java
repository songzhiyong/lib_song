 ...
 * <p>
 * Views given to the Gallery should use {@link GlideView.LayoutParams} as their
 ...
 */
// @Widget
public class GlideView extends AbsSpinner implements
		GestureDetector.OnGestureListener {

	protected static final String TAG = "GlideView";
	...
	private boolean mIsTrackScrollToLeft = false;
	...
	public GlideView(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		// //Log.d("l99",
		// String.format("GlidView.onSizeChanged(w=%d, h=%d, oldw=%d, oldh=%d)",
		// w, h, oldw, oldh));
		mCenterOfGallery = (w - getPaddingLeft() - getPaddingRight()) / 2
				+ getPaddingLeft();
	}
	...
	protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
		 ...
		 */
		return new GlideView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// //Log.d("l99",
		// String.format("GlidView.onMeasure(widthMeasureSpec=%d, heightMeasureSpec=%d)",
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		// //Log.d("l99",
		// String.format("GlidView.onLayout(changed=%s, l=%d, t=%d, r=%d, b=%d)",
		// changed, l, t, r, b));
		...
		if (null != extremeChild) {
			...
			if (mIsTrackScrollToLeft) {
				if (extremeChildCenter < galleryCenter) {
					// The extreme child is past his boundary point!
					scrollAmount = galleryCenter - extremeChildCenter;
				}
			} else {
				if (extremeChildCenter > galleryCenter) {
					// The extreme child is past his boundary point!
	...
	private void onFinishedMovement() {
		...
		invokeFinishedMoveListener();
	}

	private void invokeFinishedMoveListener() {
		boolean left = false;
		boolean right = false;

		int extremeItemPosition = mItemCount - 1;
		View extremeChild = getChildAt(extremeItemPosition - mFirstPosition);
		if (null != extremeChild && extremeChild.getRight() <= getWidth()) {
			// The extreme child is past his boundary point!
			right = true;
		}

		extremeItemPosition = 0;
		extremeChild = getChildAt(extremeItemPosition - mFirstPosition);
		if (null != extremeChild && extremeChild.getLeft() >= 0) {
			// The extreme child is past his boundary point!
			left = true;
		}

		if (null != mFinishMoveListener) {
			mFinishMoveListener.onFinishedMovement(left, right);
		}
	}

	public void smoothMove2NextPage() {
		View child = getChildAt(getChildCount() - 1);
		if (null != child) {
			int distance = getWidth() - child.getRight();
			if (mItemCount > mFirstPosition + getChildCount()
					&& getCenterOfView(child) < getWidth()) {
				distance -= (child.getWidth() + mSpacing);
			}
	...
	public void smoothMove2PrevPage() {
		View child = getChildAt(0);
		if (null != child) {
			int distance = 0 - child.getLeft();
			if (0 < mFirstPosition && getCenterOfView(child) > 0) {
				distance += child.getWidth() + mSpacing;
			}
			mFlingRunnable.startUsingDistance(distance);
		}
	...
	public boolean onSingleTapUp(MotionEvent e) {
		if (mDownTouchPosition >= 0) {
			...
			// An item tap should make it selected.
	...
	private boolean performItemSelect(int childPosition) {
		if (childPosition != mSelectedPosition) {
			View oldSelectedChild = getSelectedView();
			if (null != oldSelectedChild) {
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
			float velocityY) {
		if (0 == mFirstPosition && mItemCount == getChildCount())
			return false;

		mIsTrackScrollToLeft = velocityX < 0;
		// //Log.d("l99", "onFling(mIsTrackScrollToLeft) = " +
